...
OUTPUT_JSON  = "results/inference/comparisons/raw_captions_sft_patch_inference.json"
...
p(SEP)
p("SFT_PATCH_GRPO — ZERO-SHOT INFERENCE — 5 videos")
...
for vid in TARGET_IDS:
    ...
    assert len(paths) == 4, f"Missing keyframes for {vid}: found {paths}"
...
p(f"Loading SFT patch adapter from {PATCH_PATH} ...")
...
for vid in TARGET_IDS:
    ...
    ref     = ref_map.get(vid, "")
    results[vid] = {"gemma_ref": ref, "sft_patch": caption}
...
output = {
    ...
    "results": {
        vid: {
            ...
            "sft_patch": results[vid]["sft_patch"],
        }
        for vid in TARGET_IDS
    }
}
