#!/usr/bin/env python3
"""
SFT Validation Run — P16 BLV Project
...
_ap.add_argument('--epochs', type=int, default=3)
_args = _ap.parse_args()


# ── Logging ────────────────────────────────────────────────────────────────
PROJECT_ROOT = Path.home() / "p16_blv"
log_file     = PROJECT_ROOT / "logs/training_logs/sft_validation.log"
...
# 15% sample, seed=42
random.seed(42)
val_indices = random.sample(range(len(all_captions)), int(len(all_captions) * 0.15))
val_sample  = [all_captions[i] for i in val_indices]
log.info(f"Validation sample size: {len(val_sample)} / {len(all_captions)} total")

# Hold-out positions (within val_sample list)
HOLDOUT_POSITIONS = [0, 100, 250, 400, min(499, len(val_sample) - 1)]
holdout_set       = set(HOLDOUT_POSITIONS)
holdout_samples   = [val_sample[i] for i in HOLDOUT_POSITIONS if i < len(val_sample)]
training_samples  = [s for i, s in enumerate(val_sample) if i not in holdout_set]
...
# ── Datasets ───────────────────────────────────────────────────────────────
CKPT_DIR = PROJECT_ROOT / "models/student/sft_validation_checkpoint"
...
print("\n" + "=" * 60)
print("VALIDATION REPORT — SFT SANITY CHECK")
print("=" * 60)
