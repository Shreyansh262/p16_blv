#!/usr/bin/env python3
"""
Inference script for PaliGemma2-3B.
PaliGemma uses prefix-LM style -- no chat template.
"""
...
print(f"Loading PaliGemma2-3B on GPU {args.gpu}...")
...
def run_paligemma(image: Image.Image, prompt: str,
...
if args.task == "ocr":
    ...
    for i, sample in enumerate(ds):
        ...
        prompt = f"answer {sample['question']}"
        pred   = run_paligemma(sample["image"], prompt)
    ...
    for i, item in enumerate(data):
        ...
        pred   = run_paligemma(img, prompt, 32)
