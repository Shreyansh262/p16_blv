#!/usr/bin/env python3
"""Inference script for moondream2."""
...
if not Path(args.model).exists():
    raise FileNotFoundError(f"Not found: {args.model}")

print(f"Loading moondream2 on GPU {args.gpu}...")
...
if args.task == "ocr":
    ...
    for i, s in enumerate(ds):
        ...
        pred = run(s["image"], s["question"] + " Answer concisely.")
        results.append({"id":i,"question":s["question"],"gt":gt,"pred":pred})
        if i % 100 == 0: print(f"[{i}/{len(ds)}] {pred[:60]}", flush=True)
