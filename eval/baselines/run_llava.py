#!/usr/bin/env python3
"""
Inference script for LLaVA-1.5-7B.
Uses LlavaForConditionalGeneration with chat template.
"""
...
print(f"Loading LLaVA-1.5-7B on GPU {args.gpu}...")
...
def run_llava(image: Image.Image, question: str,
    ...
    return processor.decode(ids[0][input_len:], skip_special_tokens=True).strip()
...
if args.task == "ocr":
    ...
    for i, sample in enumerate(ds):
        ...
        pred = run_llava(sample["image"], sample["question"] + " Answer concisely.")
    ...
    for i, item in enumerate(data):
        ...
        pred = run_llava(img, item["question"] +
