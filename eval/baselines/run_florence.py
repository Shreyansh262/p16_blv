#!/usr/bin/env python3
"""
Inference script for Florence-2-large.
Florence-2 uses task tokens like <OCR>, <VQA> — prefix-LM style.
"""
...
print(f"Loading Florence-2-large on GPU {args.gpu}...")
...
def run_florence(image: Image.Image, prompt: str,
    ...
    if isinstance(parsed, dict):
        ...
        if isinstance(val, dict):
            val = val.get("transcriptions", [""])[0] if "transcriptions" in val else str(val)
...
if args.task == "ocr":
    ...
    for i, sample in enumerate(ds):
        ...
        pred = run_florence(sample["image"], "<OCR>")
    ...
    for i, item in enumerate(data):
        ...
        prompt = f"<VQA> {item['question']}"
        pred = run_florence(img, prompt, 64)
